...
void preProcess(cv::Mat &resize_img,std::vector<float>& npu_input) {
  // cv::Mat resize_img;
...
std::map<int,std::list<Detect2dObject>> filterBox(std::vector<std::vector<float>>& npu_outputs) {
  std::map<int,std::list<Detect2dObject>> m;
  for (size_t n = 0; n < npu_outputs.size(); n++) {
    std::vector<float> output(npu_outputs[n]);
...
std::map<int,std::list<Detect2dObject>> postProcess(std::vector<std::vector<float>>& npu_outputs) {
  auto m = filterBox(npu_outputs);
...
int main(){
  ...
  std::vector<float> npu_input(3 * resize_w * resize_h);
  
  preProcess(resize_img,npu_input);
  saveBin("./data/npu_input.bin",(const char *)npu_input.data(),3 * resize_w * resize_h*sizeof(float));

  // TODO:: npu_run

  std::vector<std::vector<float>> npu_outputs;
  ...
  std::map<int,std::list<Detect2dObject>> m = postProcess(npu_outputs);
